...
for (int rowi = 1; rowi < lines.Count - 1; rowi++)
{
    for (int coli = 1; coli < lines[0].Length - 1; coli++)
    {
        ...
        CheckIfCanSee(Reverse(lines[rowi][(coli + 1)..]), (rowi,coli));
        ...
        CheckIfCanSee(Reverse(completeCOl[(rowi + 1)..]), (rowi,coli));
    }
}
...
Console.WriteLine();
Console.WriteLine($"{alwaysVisible} + {internalVisibleList.Count}");
Console.WriteLine(alwaysVisible + internalVisibleList.Count);